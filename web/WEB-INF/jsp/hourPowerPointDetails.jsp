...
<tag:eazyCorePage dwr="HourPowerPointDetailsDwr" js="/resources/view.js" onload="init">
...
function DrawHourPowerChart(){

     HourPowerPointDetailsDwr.init(function(data){
	             ...
	             var  EnergyItemName;
	              ...
	              for ( var i = 0; i < num; i++){
	                    x[i] = pointDateList[i].substring(11,16);
	             ...
	             //  options.series[0].name = series.name; 
	               options.series[0].name = "时累计用电总量";
	               options.series[0].data = series.data; 
	            ///   alert(z);
	               options.series[1].data = z; 
	               options.series[1].name = "时实际用电量";
...
function UpdateHourPowerChart(startDate,endDate,companyId){

     HourPowerPointDetailsDwr.updateChart(startDate,endDate,companyId, function(data){
...
$(function(){ 
 ...
 //     $("#meterItemName").val(id);
 			UpdateHourPowerChart(startDay,endDay,meterItemName);
         }); 
...
<div class="partition"><h3>公司时耗电量趋势图</h3></div>
<div  class = "buildinglabel2">
      <label>开始时间：</label><input type="text"  id="startDay" /> 
       <label>结束时间：</label><input type="text"  id="endDay" /> 
       <label>选择电表：</label><select id="meterItemName"  style="width:160px;top:25px ;height:25px"  name="meterItemName" onchange="onchangeSelectEnergyItem(this.value)"> </select>
       <input type="submit" class="ui-button-primary " id="find_btn" value="查 询" /> 
</div>          
</div> 

 
 <div id="admin_container" style=" height: 400px; margin: 0 auto"></div> 
