...
public class FlexWatchListHandler implements UrlHandler {
    ...
    protected void prepareModel(HttpServletRequest request, Map<String, Object> model, User user) {
        ...
        List<FlexWatchList> watchLists = watchListDao.getWatchLists(user.getId());

        if (watchLists.size() == 0) {
            // Add a default watch list if none exist.
            FlexWatchList watchList = new FlexWatchList();
        ...
        int selected = 0;
        FlexWatchList selectedWatchList = watchListDao.getSelectedWatchList(user.getId());
        ...
        if (!found) {
            ...
            new FlexWatchListDao().saveSelectedWatchList(user.getId(), selected);
        }
