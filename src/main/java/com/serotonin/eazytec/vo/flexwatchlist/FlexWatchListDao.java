 ...
 * @author Matthew Lohbihler
 */
public class FlexWatchListDao extends BaseDao {
    public String generateUniqueXid() {
        return generateUniqueXid(FlexWatchList.XID_PREFIX, "watchLists");
    }
     ...
     */
    public List<FlexWatchList> getWatchLists(final int userId) {
     ...
     * Note: this method only returns basic watchlist information. No data points or share users.
     */
    public List<FlexWatchList> getWatchLists() {
        return query("select id, xid, userId, name from watchLists", new WatchListRowMapper());
    }

    public FlexWatchList getWatchList(int watchListId) {
        // Get the watch lists.
        FlexWatchList watchList = queryForObject("select id, xid, userId, name from watchLists where id=?",
                new Object[] { watchListId }, new WatchListRowMapper());
        populateWatchlistData(watchList);
        return watchList;
    }

    public void populateWatchlistData(List<FlexWatchList> watchLists) {
        for (FlexWatchList watchList : watchLists)
            populateWatchlistData(watchList);
    }

    public void populateWatchlistData(FlexWatchList watchList) {
        if (watchList == null)
            return;
     ...
     * Note: this method only returns basic watchlist information. No data points or share users.
     */
    public FlexWatchList getWatchList(String xid) {
        return queryForObject("select id, xid, userId, name from watchLists where xid=?", new Object[] { xid },
                new WatchListRowMapper(), null);
    }

    public FlexWatchList getSelectedWatchList(int userId) {
        FlexWatchList watchList = queryForObject("select w.id, w.xid, w.userId, w.name "
    ...
    class WatchListRowMapper implements RowMapper<FlexWatchList> {
        @Override
        public FlexWatchList mapRow(ResultSet rs, int rowNum) throws SQLException {
            FlexWatchList wl = new FlexWatchList();
    ...
    public FlexWatchList createNewWatchList(FlexWatchList watchList, int userId) {
    ...
    public void saveWatchList(final FlexWatchList watchList) {
    ...
    // Watch list users
    //
    private void setWatchListUsers(FlexWatchList watchList) {
    ...
    void saveWatchListUsers(final FlexWatchList watchList) {
